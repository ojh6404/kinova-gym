...
class PyBullet:
    ...
    def get_base_velocity(self, body: str) -> np.ndarray:
        ...
        """
        velocity = self.physics_client.getBaseVelocity(
            self._bodies_idx[body])[0]
    ...
    def get_link_position(self, body: str, link: int) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The position, as (x, y, z).
        """
        position = self.physics_client.getLinkState(
            self._bodies_idx[body], link)[0]
        return np.array(position)
    ...
    def get_link_orientation(self, body: str, link: int) -> np.ndarray:
        ...
        """
        orientation = self.physics_client.getLinkState(
            self._bodies_idx[body], link)[1]
        return np.array(orientation)
    ...
    def get_link_velocity(self, body: str, link: int) -> np.ndarray:
        ...
        velocity = self.physics_client.getLinkState(
            self._bodies_idx[body], link, computeLinkVelocity=True)[6]
        return np.array(velocity)
    ...
    def get_link_angular_velocity(self, body: str, link: int) -> np.ndarray:
        ...
        angular_velocity = self.physics_client.getLinkState(
            self._bodies_idx[body], link, computeLinkVelocity=True)[7]
        return np.array(angular_velocity)
    ...
    def set_base_pose(self, body: str, position: np.ndarray, orientation: np.ndarray) -> None:
        ...
        if len(orientation) == 3:
            orientation = self.physics_client.getQuaternionFromEuler(
                orientation)
    ...
    def control_joints_pos(self, body: str, joints: np.ndarray, target_angles: np.ndarray, forces: np.ndarray, Kp: np.ndarray, Kd: np.ndarray) -> None:
    ...
    def create_box(
        ...
        specular_color = specular_color if specular_color is not None else np.zeros(
            3)
        visual_kwargs = {
        ...
        if texture is not None:
            texture_path = os.path.join(
                kinova_gym.assets.get_data_path(), texture)
    ...
    def create_plane(self, z_offset: float) -> None:
        ...
        self.physics_client.loadURDF(
            "plane.urdf", basePosition=[0., 0., z_offset])
